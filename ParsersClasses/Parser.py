...
class Parser():
    ...
    # only for parser
    # THIS IS ONLY FOR CSV HEADER
    __errorLimits = [0.0, 0.2, 0.5]
    __keys = ["errorLimit" + str(i) for i in __errorLimits]
    ...
    # CAIO's approach to generate beautiful graphs
    # _toleratedRelativeErrors = []
    _relativeErrorLowerLimits = []
    ...
    # OLD DUMB STYLE, and not generic enough
    # "Relative Errors <= " + str(_toleratedRelErr) + "%",
    # "Relative Errors <= " + str(_toleratedRelErr2) + "%",
    # "Jaccard",
    # "Jaccard > " + str(_toleratedRelErr) + "%",
    # "Jaccard > " + str(_toleratedRelErr2) + "%",
    # "Cubic", "Square", "Line", "Single", "Random",
    # "Cubic Err > " + str(_toleratedRelErr),
    # "Square Err > " + str(_toleratedRelErr),
    # "Line Err > " + str(_toleratedRelErr),
    # "Single Err > " + str(_toleratedRelErr),
    # "Random Err > " + str(_toleratedRelErr),
    # "Cubic Err > " + str(_toleratedRelErr2),
    # "Square Err > " + str(_toleratedRelErr2),
    # "Line Err > " + str(_toleratedRelErr2),
    # "Single Err > " + str(_toleratedRelErr2),
    # "Random Err > " + str(_toleratedRelErr2),
    ...
    def __init__(self, **kwargs):
        ...
        if parseForHistogram:
            ...
            self._relativeErrorLowerLimits = [0] * len(self.__errorLimits)

            self.__keys = ["errorLimit" + str(i) for i in self.__errorLimits]



        for per in self.__errorLimits:
            # to store all error parsed values
            self._errors["errListFiltered_" + str(per)] = []
            self._errors["relErrLowerLimit_" + str(per)] = 0
            # cubic, square, colRow, single, random
            self._locality["locality_" + str(per)] = [0, 0, 0, 0, 0]
            self._jaccardCoefficientDict["jaccardCoefficient_" + str(per)] = 0


            self._csvHeader.append("relative_errors_<=_" + str(per) + "%")
            self._csvHeader.append("jaccard_>_" + str(per) + "%")

        for per in self.__keys:
            for type in ["cubic", "square", "line", "single", "random"]:
                self._csvHeader.append(type + "_>" + str(per))
    ...
    """
    for almost all benchmarks this method must be override, because it is application dependent
    """
    ...
    """
    def __placeRelativeError(self, relError):
    ...
    # zeros in the output,
    #zero in the GOLD,
    #errors with relative errors lower than limit(toleratedRelErr),
    list of errors limited by toleratedRelErr,
    #errors with relative errors lower than limit(toleratedRelErr2),
    list of errors limited by toleratedRelErr2]
    assume errList[2] is read valued and errList[3] is expected value
    """
    ...
    def _relativeErrorParser(self, errList):
        ...
        for err in errList:
            ...
            if abs(expected) < 1e-6:
                ...
                # else:
                #     errListFiltered2.append(err)
                self.__placeRelativeError(relError)
