...
class BezierSurfaceParser(Parser):
    ...
    def parseErrMethod(self, errString):
        ...
        try:
            m = re.match(
                ".*ERR.*p: \[(\d+)\, (\d+)\].*X r\: (\d+\.\d+).* e\: (\d+\.\d+).*Y r\: (\d+\.\d+).*e\: (\d+\.\d+).*Z r\: (\d+\.\d+).*e\: (\d+\.\d+).*",
                errString)
            if m:
                px = int(m.group(1))
                py = int(m.group(2))
    ...
    def _relativeErrorParser(self, errList):
        ...
        [self._maxRelErr, self._minRelErr, self._avgRelErr,
         self._zeroOut, self._zeroGold, self._countErrors] = [0] * 6
    ...
    def setSize(self, header):
        ...
        if m:
            ...
            self._size = str(self._i) + "_" + str(self._g) + "_" + str(self._a) + "_" + str(self._t) + "_" + str(
                self._n)
        else:
