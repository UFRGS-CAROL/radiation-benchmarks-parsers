...
#----------------------------- 07/06/2018 -----------------------------------
# 1- The log given by CHIPIR is not a csv, replace spaces and tabs with ; 
# 2- Not able to check correctness due to absense of Distance Factor 
#       and Phisical Magic factor
#----------------------------------------------------------------------------
...
def get_fluency_flux(start_dt, end_dt, file_lines, factor):
    # file_lines eh a corrtete
    ...
    for l in file_lines:    # file_lines is the neutron_log/current_log given by CHIPIR
        ...
        if first_curr_integral is not None:
            if curr_integral == last_curr_integral:
...
def main():
    ...
    factor = MAGIC_THERMAL_VALUE # float(sys.argv[3])
        ...
        for bench in grouped_benchmarks:
            ...
            while i < len(lines) - 1:
                ...
                row = [start_dt.ctime(), end_dt.ctime(), machine, bench, header_info,
                       ...
                       cross_section_crash_acc_time, time_beam_off_acc_time, MAGIC_THERMAL_VALUE]
