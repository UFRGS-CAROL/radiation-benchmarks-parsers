...
def getFluenceFluxDiamond(startDT, endDT):
	...
	for l in fileLines:
		...
		if curDt > endDT:
			...
			print "counter_40 " + str(counter_40) + " first " + str(first_counter_40) + " res " + str(counter_40 - first_counter_40)
...
while i < len(lines):
	...
	row = [startDT.ctime(), endDT.ctime(), (i-j+1), sdcS, accTimeS, abortZeroS, flux, fluxAccTime, fluxDiamond, fluxAccTimeDiamond, fluence, fluenceAccTime, crossSection, crossSectionCrash, timeBeamOff, crossSectionAccTime, crossSectionCrashAccTime, timeBeamOffAccTime]
	writer.writerow(row)
